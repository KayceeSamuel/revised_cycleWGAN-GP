...
class CycleGANModel(BaseModel):
    ...
    #The original code for the cycleGAN
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        # Real
        pred_real = netD(real)
        loss_D_real = -torch.mean(pred_real) # Changed to WGAN Loss
        # Fake
        pred_fake = netD(fake.detach())
        loss_D_fake = torch.mean(pred_fake) # Changed to WGAN Loss
        # Combined loss
        loss_D = (loss_D_real + loss_D_fake) * 0.5
    ...
    def backward_G(self):
        ...
        #self.loss_G_A = -self.criterionGAN(self.netD_A(self.fake_B)) # Changed to WGAN Loss
        # GAN loss D_B(G_B(B))
        self.loss_G_B = self.criterionGAN(self.netD_B(self.fake_A), True) # Changed to WGAN Loss
        
        #self.loss_G_B = -self.criterionGAN(self.netD_B(self.fake_A)) # Changed to WGAN Loss
